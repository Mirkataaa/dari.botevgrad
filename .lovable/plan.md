...
6. **Чат 6 ✅:** Финал — мигрирани последните **7 frontend файла** (Profile, EditCampaign, CampaignWizard, AdminCampaigns, AdminCampaignPreview, AdminComments, AdminDraftReview). Добавени липсващи **backend endpoints** в `admin.ts` (campaigns CRUD, drafts/comments admin, single campaign+donations, single draft) и в `campaigns.ts` (PATCH /:id, POST /:id/drafts, GET /:id/rejections, GET /:id/pending-draft, GET /me/pending-drafts). Добавен `routes/og.ts` (заменя Supabase `og-preview` edge function — UA detection + OG meta-tags за crawler-и). Локални `lib/types.ts` (CampaignCategory enum). Поправен `ShareWidget` да използва `/og/:id`. Финализирани deploy artefact-и: `deploy/certbot-setup.md`, `deploy/systemd/dari-backend.service`, `deploy/systemd/dari-email-worker.service`. ZIP: `dari-botevgrad-vps.zip`
